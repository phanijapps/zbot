...
impl TriggerCondition {
    /// Check if the trigger condition should activate
...
"#;
    ...
    #[test]
    fn test_context_editing_config_default() {
        let config = ContextEditingConfig::default();
        assert!(!config.enabled);
        assert_eq!(config.trigger_tokens, 10000);
        assert_eq!(config.keep_tool_results, 3);
    }
}
