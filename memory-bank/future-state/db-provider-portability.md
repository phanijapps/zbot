...
| **D6 (partial — shipped)** | `stores/zero-stores-sqlite::kg` re-exports `GraphStorage`, `OrphanCandidate`, `GraphTraversal`, `SqliteGraphTraversal`, `TraversalNode`, `CausalEdge`, `CausalEdgeStore` from `services/knowledge-graph`. Callers can import from the "right" location now; physical file move is deferred. | 1 hr (done) | Re-exports compile; old import paths still work |
| **D6b (deferred)** | Physically move `services/knowledge-graph/src/storage.rs` (3,212 LoC), `traversal.rs` (440 LoC), `causal.rs` (219 LoC) into `stores/zero-stores-sqlite/src/kg/`. Update internal cross-references. `services/knowledge-graph` keeps only resolver + extractor + types + service (business logic). 7 external import sites still work via the re-export from D6. | 8-12 hr | Subagent-friendly mechanical pass with conformance + KG tests as verification |
