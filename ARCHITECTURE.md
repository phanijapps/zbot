...
The project is now structured as a **Cargo workspace** with a reusable `agent-runtime` library crate that provides the core agent execution framework.
...
├── crates/
│   └── agent-runtime/         # Reusable agent execution library
│       ├── src/
│       │   ├── types/         # Shared types (messages, events)
│       │   ├── llm/           # LLM client abstraction
│       │   ├── tools/         # Tool system (registry, built-in tools)
│       │   ├── mcp/           # MCP protocol support
│       │   ├── middleware/    # Middleware pipeline
│       │   ├── executor/      # Core agent executor
│       │   └── logging/       # Structured logging
│       └── Cargo.toml
└── src-tauri/                 # Tauri application
    └── src/
        └── domains/
            └── agent_runtime/  # Tauri-specific adaptations
```
...
│  │                                                           │   │
│  │  ┌────────────────────────────────────────────────────┐  │   │
│  │  │        agent-runtime Library (Reusable)            │  │   │
│  │  │  ┌──────────────────────────────────────────┐      │  │   │
│  │  │  │ Types: ChatMessage, StreamEvent, ToolCall │      │  │   │
│  │  │  ├──────────────────────────────────────────┤      │  │   │
│  │  │  │ LLM Client (OpenAI-compatible API)        │      │  │   │
│  │  │  ├──────────────────────────────────────────┤      │  │   │
│  │  │  │ Tools: Registry + 9 Built-in tools        │      │  │   │
│  │  │  ├──────────────────────────────────────────┤      │  │   │
│  │  │  │ MCP: Manager + HTTP/Stdio/SSE clients    │      │  │   │
│  │  │  ├──────────────────────────────────────────┤      │  │   │
...
│  │  │  - Config file loading (YAML, providers.json)    │  │   │
│  │  │  - FileSystemContext (AppDirs integration)       │  │   │
