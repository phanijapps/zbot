...
// `zbot-stores-sqlite`) so the `ConversationStore` trait in
// `zbot-stores-traits` can hand back conversation history without forcing
...
/// (e.g. `zbot-stores-sqlite` parses `tool_calls` JSON, an HTTP gateway may
...
pub struct Message {
    ...
    /// `zbot-stores-sqlite::ConversationRepository` for the canonical
    /// stored format.
