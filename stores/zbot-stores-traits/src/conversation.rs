...
// (`docs/architecture/future-state/persistence-readiness-design.md`). This trait
...
//   — that would force `zbot-stores-traits` to depend on `agent-runtime`
...
use zbot_stores_domain::Message;

/// Backend-agnostic interface for the conversation message stream.
///
/// Mirrors a subset of `zbot_stores_sqlite::ConversationRepository`'s public
